...
class ClientHandler implements Runnable {
    ...
    HashMap<InetAddress,Integer> tabela;

    // Constructor
    public ClientHandler(DatagramSocket socket,DatagramPacket DpReceive,List<InetAddress> baseDados,String threadID,HashMap<InetAddress,Integer> tabela )
    {
        this.ds = socket;
    ...
    public void run()
    {
        ...
        switch(messageType){
            case "m1": 	
                        ...
                        System.err.println("todos os nos foram notificados");
            ...
            case "m3":
                        byte[] ipConfirmacaoBytes = Arrays.copyOfRange(receive, 8, 12);
                        System.out.println(InetAddress.getByAddress(ipConfirmacaoBytes).toString());
                        tabela.put(InetAddress.getByAddress( ipConfirmacaoBytes),-1);
...
Map<String, ClientHandler> mapFromFile
= HashMapFromTextFile();
...
/*public static Map<String, ClientHandler> HashMapFromTextFile()
{

Map<String, ClientHandler> map
...
// first part is name, second is handler
String name = parts[0].trim();
ClientHandler handler = parts[1].trim().getClass().getName();

// put name, handler in HashMap if they are
// not empty
if (!name.equals("") && !handler.equals(""))
